...
router.get("/scrape", function (req, res) {
  ...
  axios.get("http://www.cunard.com/cruise-search/book-a-cruise/results/?tids=TRAN").then(function (response) {
    ...
    $("div.module.pricingContainer.cruiseSearchResults").each(function (i, element) {
      ...
      resultCunard.name = $(this).children().children().children().find("h2.resultHeading").text();
      resultCunard.price = $(this).children().children().children().children().children().children().children().find("div.farePrice").text();
      resultCunard.date = $(this).children().children().children().find("h2.resultHeading").find("ul.resultOverview").children().text().split(' , ');
      // result.summary = $(this).children().children().children().find("noscript").text().slice(165, 400).split("alt=");

      console.log("resultCunard", resultCunard);
      ...
      router.route("/")
        .get(cruisesController.findAllCunard)
        .post(cruisesController.createCunard);
